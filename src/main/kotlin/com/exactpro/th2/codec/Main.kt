...
class CodecCommand : CliktCommand() {
    ...
    override fun run() {
        try {
            ...
            val storeEventFunc: (Event, EventID?) -> EventID = { event, parentId ->
                eventRouter.runCatching {
                    val protoEvent = event.toProto(parentId ?: rootEventId)
                    sendAll(
                        EventBatch.newBuilder().addEvents(protoEvent).build(),
                        QueueAttribute.PUBLISH.toString(),
            ...
            val storeRootEventFunc: (Event, EventID?) -> EventID = { event, _ ->
                eventRouter.runCatching {
                    val protoEvent = event.toProto(rootEventId)
                    sendAll(
                        EventBatch.newBuilder().addEvents(protoEvent).build(),
                        QueueAttribute.PUBLISH.toString(),
                        QueueAttribute.EVENT.toString()
