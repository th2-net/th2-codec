...
class SyncCodecTest {
    ...
    fun `enable external queue routing test`() {
        TestSyncCodec(messageRouter, eventRouter, processor, CODEC_ROOT_ID, true).apply {
            start(SOURCE_ATTRIBUTE, TARGET_ATTRIBUTE)
        }.use { codec ->
            for (batch in ORIGIN_BATCH_SET) {
                codec.handler(null, batch)
            ...
            for (completeBatch in completeBatches) {
                codec.handler(null, completeBatch)
                verify(
                    messageRouter,
                    times(1).description("Send ${shortDebugString(completeBatch)} complete batch")
                ).sendExclusive(eq(BATCH_METADATA.externalUserQueue), eq(completeBatch))
                verify(
                    processor,
            ...
            codec.handler(null, incompleteBatch)
            verify(
                messageRouter,
    ...
    fun `disable external queue routing test`() {
        TestSyncCodec(messageRouter, eventRouter, processor, CODEC_ROOT_ID, false).apply {
            start(SOURCE_ATTRIBUTE, TARGET_ATTRIBUTE)
        }.use { codec ->
    ...
    companion object {
        private const val CODEC_ROOT_ID = "root-event-id"
