...
class ProcessorTest {
    ...
    fun `simple test - decode`(protocol: Protocol) {
        val processor = UniversalCodec(TestCodec(false), ORIGINAL_PROTOCOLS, process = DECODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
    ...
    fun `multiple protocols test - decode`(protocol: Protocol) {
        ...
        val processor = UniversalCodec(TestCodec(false), originalProtocols, process = DECODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
        val batch = getNewBatchBuilder(protocol, BOOK_NAME_A, SESSION_GROUP_NAME)
            .addNewParsedMessage(type = MESSAGE_TYPE, protocol = ORIGINAL_PROTOCOL)
            .addNewRawMessage(protocol = WRONG_PROTOCOL)
            .addNewRawMessage(protocol = WRONG_PROTOCOL)
    ...
    fun `simple test - encode`(protocol: Protocol) {
        val processor = UniversalCodec(TestCodec(false), ORIGINAL_PROTOCOLS, process = ENCODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
        val batch = getNewBatchBuilder(protocol, BOOK_NAME_A, SESSION_GROUP_NAME)
            .addNewParsedMessage(type = MESSAGE_TYPE, protocol = ORIGINAL_PROTOCOL)
            .addNewParsedMessage(type = MESSAGE_TYPE, protocol = WRONG_PROTOCOL, )
            .addNewParsedMessage(type = MESSAGE_TYPE, protocol = WRONG_PROTOCOL)
    ...
    fun `multiple protocols test - encode`(protocol: Protocol) {
        ...
        val processor = UniversalCodec (TestCodec(false), originalProtocols, process = ENCODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
    ...
    fun `error message on thrown - encode`(protocol: Protocol) {
        val processor = UniversalCodec (TestCodec(true), ORIGINAL_PROTOCOLS, process = ENCODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
    ...
    fun `error message on thrown - decode`(protocol: Protocol) {
        val processor = UniversalCodec (TestCodec(true), ORIGINAL_PROTOCOLS, process = DECODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
        val batch = getNewBatchBuilder(protocol, BOOK_NAME_A, SESSION_GROUP_NAME)
            .addNewRawMessage(
                id = MESSAGE_ID,
    ...
    fun `error message on failed protocol check - decode`(protocol: Protocol) {
        ...
        val batch = getNewBatchBuilder(protocol, BOOK_NAME_A, SESSION_GROUP_NAME)
            ...
            .addNewRawMessage(eventId = CODEC_EVENT_ID_BOOK_A.copy(id = UUID.randomUUID().toString()))
            .build()
    ...
    fun `multiple protocol test - decode`(protocol: Protocol) {
        val processor = UniversalCodec(TestCodec(true), setOf("xml", "json"), process = DECODE, protocol = protocol, eventProcessor = EventProcessor(CODEC_EVENT_ID_BOOK_A.toProto()) {}, config = config)
        val batch = getNewBatchBuilder(protocol, BOOK_NAME_A, SESSION_GROUP_NAME)
            .addNewRawMessage(
                eventId = CODEC_EVENT_ID_BOOK_A.copy(id = UUID.randomUUID().toString()),
                protocol = "xml"
